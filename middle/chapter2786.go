		...
		if nums[i]%2 == nums[i-1]%2 {
			...
			if nums[i]%2 == 1 {
				if nums[odd]%2 == nums[i]%2 {
					dp[i] = maxInt64(dp[i-1]-int64(x), dp[odd]) + int64(nums[i])
				} else {
				...
				if nums[even]%2 == nums[i]%2 {
					dp[i] = maxInt64(dp[i-1]-int64(x), dp[even]) + int64(nums[i])
				} else {
					dp[i] = dp[i-1] + int64(nums[i]) - int64(x)
				}
				even = i
...
func maxScoreII(nums []int, x int) int64 {
	...
	for i := 1; i < len(nums); i++ {
		...
		if nums[i]%2 == 0 {
			dp[i][0] = maxInt64(dp[i-1][0]+int64(nums[i]), dp[i-1][1]+int64(nums[i])-int64(x))
		} else {
			dp[i][1] = maxInt64(dp[i-1][1]+int64(nums[i]), dp[i-1][0]+int64(nums[i])-int64(x))
		}
	}
	return maxInt64(dp[len(dp)-1][0], dp[len(dp)-1][1])
}
